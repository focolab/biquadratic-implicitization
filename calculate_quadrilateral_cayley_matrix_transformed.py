...
P_x = P_x.subs(p00_x,0)
P_y = P_y.subs(p00_y,0)
P_z = P_z.subs(p00_z,0)
P_x = P_x.subs(p02_x,1)
P_y = P_y.subs(p02_y,0)
P_z = P_z.subs(p02_z,0)
P_z = P_z.subs(p20_z,0)
...
# now take determinant of first matrix to find cayley polynomial
...
with open('biquadratic_quadrilateral_cayley_matrix_transformed_mathematica.txt', 'w') as f:
